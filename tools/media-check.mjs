...
const hb = (text) => p.locator('.m-live .m-ihdr button, .m-rs__chat button', { hasText: new RegExp(`^\\s*${text}\\s*$`) });

// ── Spot
await nav('Spot');
out.spotCards = await count('.m-spot-card');
await p.locator('.m-spot-card__thumb').first().click(); await p.waitForTimeout(500);
out.spotPlayer = { open: await count('.m-spotp'), title: await p.locator('.m-spotp .m-rs__name').textContent(), frame: await count('.m-spotp .m-frame'), timeline: await count('.m-spotp .m-tl') };
...
await p.locator('.m-spotp .m-ihdr__panels button').click(); await p.waitForTimeout(300);
await p.locator('.m-jrn__tab', { hasText: 'Comments' }).click(); await p.waitForTimeout(200);
out.comments = { panel: await count('.m-spotp .m-jrn'), rows: await count('.m-rs__comment') };
await p.locator('.m-rs__composer input').fill('Looks reproducible on staging too.');
await p.locator('.m-rs__send').click(); await p.waitForTimeout(200);
out.commentsAfterPost = await count('.m-rs__comment');
out.badge = await p.locator('.m-jrn__tab', { hasText: 'Comments' }).locator('.m-rs__badge').textContent();
await p.screenshot({ path: `${SHOTS}/spot-player.png` });
await p.locator('.m-rs__aside-close button').click(); await p.waitForTimeout(200);
out.panelClosedByX = (await count('.m-spotp .m-jrn')) === 0;
await p.locator('.m-spotp .m-ihdr__panels button').click(); await p.waitForTimeout(200);
await p.locator('.m-jrn__tab', { hasText: 'Activity' }).click(); await p.waitForTimeout(300);
out.activity = await count('.m-rs__act');
await p.locator('.m-rs__act').last().click(); await p.waitForTimeout(200);
out.seekedTo = await p.evaluate(() => document.querySelector('.m-tl__clock')?.textContent);
await p.locator('.m-spotp .m-dt [role="tab"]', { hasText: 'Console' }).click(); await p.waitForTimeout(300);
out.console = { open: await count('.m-spotp .m-dt.is-open'), rows: await count('.m-spotp .m-dt.is-open [class*="row"]') };
await p.getByRole('button', { name: 'Manage access' }).click(); await p.waitForTimeout(300);
out.access = await count('.m-rs__access');
await p.keyboard.press('Escape'); await p.waitForTimeout(150);
await p.locator('.m-spotp .m-rs__back').click(); await p.waitForTimeout(400);
...
out.live = { open: await count('.m-live'), name: await p.locator('.m-live .m-rs__name').textContent(), liveTag: await p.locator('.m-player__live').getByText('LIVE').count(), remoteDisabled: await hb('Remote control').isDisabled(), annotateHidden: (await hb('Annotate').count()) === 0 };
...
out.onCall = { chat: await count('.m-rs__chat'), endBtn: await hb('End').count(), annotate: await hb('Annotate').count(), remoteEnabled: !(await hb('Remote control').isDisabled()) };
...
out.annotating = await count('.m-rs__ink');
await p.locator('.m-live .m-dt [role="tab"]', { hasText: 'Console' }).click(); await p.waitForTimeout(300);
out.liveConsole = await count('.m-live .m-dt.is-open');
await p.screenshot({ path: `${SHOTS}/live-page.png` });
const before = await p.locator('.m-player__elapsed').textContent(); await p.waitForTimeout(1200);
out.elapsedTicks = before !== (await p.locator('.m-player__elapsed').textContent());
await hb('End').first().click(); await p.waitForTimeout(300);
out.afterEnd = { chat: await count('.m-rs__chat'), callBtn: await hb('Call').count(), remoteDisabled: await hb('Remote control').isDisabled() };
await p.locator('.m-live .m-rs__back').click(); await p.waitForTimeout(400);
