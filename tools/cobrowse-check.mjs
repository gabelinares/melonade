...
t('COBROWSE: newest first by default, and the header says so', s.on === 1 && s.rows[0][1].startsWith('2m'), `${s.on} on, first ${s.rows[0]?.[1]}`);
...
t('COBROWSE: click Duration → shortest first', durs.every((d, i) => i === 0 || d >= durs[i - 1]), durs.join(','));
await p.locator('.m-ss__table th', { hasText: 'Duration' }).click();
await p.waitForTimeout(300);
s = await table();
const durs2 = s.rows.map((r) => sec(r[2]));
t('COBROWSE: click again → longest first', durs2.every((d, i) => i === 0 || d <= durs2[i - 1]), durs2.join(','));
...
t('SESSIONS: click Events → ascending figures', ev.every((d, i) => i === 0 || d >= ev[i - 1]), ev.join(','));
