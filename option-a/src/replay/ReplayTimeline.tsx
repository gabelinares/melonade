...
export function ReplayTimeline({ clock, markers, failure, trailing }: ReplayTimelineProps) {
  ...
  return (
    <div className="m-tl">
      ...
      <span className="m-tl__clock m-mono">{formatClock(clock.at)}</span>
      ...
      >
        <span className="m-tl__rail" aria-hidden="true" />
        <span className="m-tl__fill" style={{ width: `${pct}%` }} aria-hidden="true" />
        ...
        {markers.map((m, i) => (
          ...
          >
            <button
              type="button"
              className={`m-tl__mark m-tl__mark--${m.kind}${clock.at >= m.at ? ' is-past' : ''}`}
        ...
        <span className="m-tl__head" style={{ left: `${pct}%` }} aria-hidden="true" />
      </div>

      <span className="m-tl__clock m-tl__clock--total m-mono">{formatClock(clock.duration)}</span>

      {failure && (
        <button
          type="button"
          className="m-tl__jump"
      ...
      <div className="m-tl__speeds" role="group" aria-label="Playback speed">
        {REPLAY_SPEEDS.map((s) => (
          <button
            key={s}
            type="button"
            className={`m-tl__speed${clock.speed === s ? ' is-on' : ''}`}
