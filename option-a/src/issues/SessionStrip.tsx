...
export function SessionStrip({
  ...
  if (density === 'cards') {
    return (
      <section className="m-strip m-strip--cards" aria-label="Sessions that hit this issue">
        ...
        <header className="m-strip__head">
          {/* Graphite writes a section head as sentence-case text with the
              count trailing it in the muted suffix this app uses everywhere
              else, not as an uppercase eyebrow. `x of n` whenever the band is
              showing fewer than the issue has, whether that is a filter or the
              cap doing it: a bare "3" on an issue with a hundred and thirty
              would be the band shortening its own label by lying about the
              issue. */}
          <h2 className="m-strip__label">
            ...
            {shown.length === n ? (
              ...
              <span className="m-strip__of">{shown.length} of {n}</span>
            )}
          </h2>
          {!searching && <p className="m-strip__hint">Pick the one you want to watch.</p>}
  ...
  return (
    <section className="m-strip m-strip--row" aria-label="Sessions that hit this issue">
      ...
      <span className="m-strip__position">
        {shown.length} of {n}
      </span>

      <div
        className={`m-strip__chips${edges.left ? ' has-left' : ''}${edges.right ? ' has-right' : ''}`}
        ref={railRef}
      >
        {shown.map((s, i) => (
          <button
          ...
          >
            ...
            <span className="m-schip__dur m-mono">{s.dur}</span>
          </button>
        ))}
      </div>
      ...
      <div className="m-strip__controls">
        <IconButton
          icon={<Plus size={15} />}
          label={
            more > 0
