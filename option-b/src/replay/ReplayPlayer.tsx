...
export function ReplayPlayer({ issue, session, clock }: ReplayPlayerProps) {
  ...
  return (
    <section className="b-player" aria-label="Session replay">
      <div className="b-player__chrome">
        <span className="b-player__lights" aria-hidden="true">
          <i /><i /><i />
        </span>
        <span className="b-player__url m-mono m-truncate">{replayUrl(issue)}</span>
        <span className="b-player__env">
          {session.browser} on {session.os} · {session.loc}
        </span>
      </div>

      <div className="b-player__stage">
        ...
        <p className="b-player__caption" aria-live="polite">
