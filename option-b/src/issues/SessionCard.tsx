...
export function SessionCard({ session, index, active, onOpen }: SessionCardProps) {
  ...
  return (
    <button
      type="button"
      className={`b-scard${active ? ' is-active' : ''}`}
      onClick={onOpen}
      aria-label={`Watch session ${index + 1}: ${session.email}`}
    >
      <span className="b-scard__shot">
        <ReplayFrame className="b-scard__frame" variant="still" markerIndex={at} />
        ...
        <span className="b-scard__dur m-mono">{session.dur}</span>
        ...
        {moment && (
          <span className="b-scard__moment m-truncate" title={moment.label}>
            {moment.label}
          </span>
        )}

        <span className="b-scard__play" aria-hidden="true">
          <Play size={14} />
        </span>
      </span>

      <span className="b-scard__variation">{session.variation}</span>

      <span className="b-scard__foot">
        <span className="b-scard__who m-truncate">{session.email}</span>
        <Chip quiet>{session.plan}</Chip>
      </span>
      ...
      <span className="b-scard__meta m-truncate">
