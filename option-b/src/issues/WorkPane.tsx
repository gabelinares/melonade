...
export interface WorkPaneProps {
  ...
  /** the issue queue's state, so the header can carry its toggle and the pane
   *  can decide how much of the extra width to actually use */
  queueOpen: boolean;
  onToggleQueue: () => void;
...
export function WorkPane(props: WorkPaneProps) {
  ...
  return (
    /* `is-wide` is the pane reporting that it has the queue's width as well,
       which changes how much of it the article is allowed to use. See
       work-pane.css: the answer is not "all of it". */
    <div className={`b-work b-work--${depth}${!props.queueOpen && !watching ? ' is-wide' : ''}`}>
      <IssueHeader
        issue={issue}
        title={title}
        open={writeUpOpen}
        ...
        queueOpen={watching ? undefined : props.queueOpen}
        onToggleQueue={watching ? undefined : props.onToggleQueue}
      ...
      <div className="b-work__body">
        <div className="b-work__main">
          ...
          {watching ? (
            ...
            <ScrollArea className="b-work__scroll" type="auto">
              <IssueContext
                issue={issue}
                title={title}
                size="full"
        ...
        {watching && session && sidePanel === 'journey' && (
